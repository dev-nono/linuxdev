...
void* fnTreadLoopEvent(void * a_pThis)
{
...
//	class_id   = (argc > 4) ? (int)strtol (argv[4], NULL, 0) : LIBUSB_HOTPLUG_MATCH_ANY;
	...
	while (1)
	{
		...
		{
			printf("--- libusb_handle_events() result = %d\n",rc);
		}
	}
...
{
	int result = 0;
	pthread_t vPthreadID;

	result = pthread_create(&vPthreadID,NULL,fnTreadLoopEvent,0);

		pthread_join(vPthreadID,0);
}
